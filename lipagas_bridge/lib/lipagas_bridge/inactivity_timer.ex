defmodule LipagasBridge.InactivityTimer do
  @moduledoc """
  ...
  defp send_inactivity_reminder(conv_id, phone) do
    ...
    unless skip? do
      ...
      case LipagasBridge.Typebot.start_chat(slug, %{"SystemEvent" => "INACTIVITY_TIMEOUT", "Phone" => phone}) do
        {:ok, session_id, messages, input} ->
          # Parse the resulting visual blocks (text + buttons)
          {combined_text, image_url} = LipagasBridge.Typebot.parse_messages(messages)
          
          if combined_text != "" do
             LipagasBridge.LipagasHandler.build_and_send(phone, combined_text, image_url, input, conv_id)
             LipagasBridge.Session.set_session(phone, session_id)
          end
        _ -> :ok
      end
    end
