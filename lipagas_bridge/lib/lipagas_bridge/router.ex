defmodule LipagasBridge.Router do
  ...
  post "/webhook" do
    IO.puts("[Chatwoot Webhook] Event: #{inspect(conn.body_params["event"])}, Msg Type: #{inspect(conn.body_params["message_type"])}, Content: #{inspect(conn.body_params["content"])}")
    conn = send_resp(conn, 200, "OK")
    Task.start(fn ->
      try do
        LipagasBridge.LipagasHandler.handle(conn.body_params)
      rescue
