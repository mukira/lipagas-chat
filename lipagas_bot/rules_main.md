...
## 24. Typebot V6.1 Webhook & SSRF Debugging
...
## 25. Free-Tier AI API Optimization, Chunking & On-Demand Translation Rule
- All AI integrations rely on free-tier accounts for Groq and Gemini.
- **Token Efficiency & Chunking**: All LLM prompts MUST be optimized for token efficiency by truncating raw search or scraping context payloads down to essential fields only (~1,200 chars max) before sending to AI endpoints.
- **24-Hour Redis Caching**: Generated LLM project outputs MUST be cached in Redis with a 24-hour TTL keyed by query/location to eliminate redundant API calls for identical requests.
- **On-Demand & Persistent Translation Caching**: Text and interactive button translations MUST ONLY be generated on a strictly per-needed basis when a user interacts in that language. Every generated translation MUST be permanently stored in Redis (`pres_trans:...` and `pres_btn_trans:...`) so that all future users selecting that language reuse the cached translation with ZERO API calls.
- **Round-Robin Multi-Key Fallback**: Always enforce round-robin rotation across all available Groq and Gemini API keys with exponential backoff and zero-crash fallbacks to raw regex parsing engines.
- **Authentic Presidential Persona**: ALL LLM prompts must strictly mandate the authentic natural language, cadence, and unique speech patterns of President William Ruto (e.g., using terms like "My friend", "Let me tell you", "Bottom-up", and his deliberate, accountable tone). Generic "AI politician" or sterile corporate language is strictly forbidden.

## 26. Freeform Question Native AI Takeover & Follow-Up Rule
- **Native AI Interception**: Any freeform text input from a user that does NOT explicitly match an interactive button label (e.g. `📜 Read Manifesto`, `📊 View Scorecard`), deep switch, or reset keyword (e.g. `Ruto`, `exit`, `menu`) MUST be intercepted natively by `AIProxy.process_request/1`.
- **Authentic Response Delivery**: The response generated by AI Proxy MUST be delivered directly to the user via Meta WhatsApp API with standard WhatsApp markdown formatting (`**` converted to `*`).
- **Zero Dead-End Interactive Follow-Up**: Immediately following the AI response, the bridge MUST send an interactive button card asking the user what they wish to explore next, presenting options: `💬 Ask President Ruto` and `⬅️ Back to Main Menu`. This ensures the chat never freezes or lands in a dead end.
