...
2. **Trace the full data flow end-to-end.** Understand exactly what happens when the user performs an action — from WhatsApp → Chatwoot → bridge.js → Typebot → APIs → back. Do not guess any step.
...
6. **Handle Edge Cases Gracefully (No Crashing)**: When relying on external APIs (like Meta, Daraja, or StronPower), you must always wrap calls in `try/catch` blocks. If an API fails, provide a safe fallback so the server never crashes.
