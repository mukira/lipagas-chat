defmodule LipagasBridge.BotRouter do
  @moduledoc """
    ...
    if phone do
      ...
      cond do
        content_clean == "ruto" ->
          ...
          LipagasBridge.Session.set_active_bot(phone, "presidential-bot")
          # Force a Chatwoot status update here if necessary, or just clear Typebot session
          LipagasBridge.Session.delete_session(conv_id)

        content_clean == "joywo" ->
          IO.puts("[BotRouter] Keyword JOYWO detected for #{phone}. Switching to JoyWo Bot.")
          LipagasBridge.Session.set_active_bot(phone, "my-typebot-jpjwqnz")
          LipagasBridge.Session.delete_session(conv_id)
      ...
      active_slug = LipagasBridge.Session.get_active_bot(phone) || default_slug

      if active_slug do
        IO.puts("[BotRouter] Routing inbox_id=#{inbox_id} for #{phone} to slug=#{active_slug}")
        LipagasBridge.TypebotBotHandler.handle(payload, active_slug)
        :handled
      else
