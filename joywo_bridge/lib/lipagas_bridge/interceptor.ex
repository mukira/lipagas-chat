defmodule LipagasBridge.Interceptor do
  @moduledoc """
  ...
  """
  alias LipagasBridge.{Meta, Config}
  ...
  def process(combined_text, phone, image_urls, conv_id) do
    cond do
      ...
      String.contains?(combined_text, "[SAVE_LOCATION:") ->
        ...
        if location != "" do
          case LipagasBridge.Chatwoot.get_sender(conv_id) do
            {:ok, sender} ->
              LipagasBridge.Chatwoot.update_contact(sender.id, %{saved_location: location})
            _ -> :ok
          end
        end
        ...
        case Regex.run(~r/\[ADD_TO_CART:([\d.]+)\|([^\]]+)\]/, combined_text) do
          [_, amt_str, lbl_str] ->
            ...
            enriched_lbl = if base_lbl == "Gas Tokens" do
              meter = case LipagasBridge.Chatwoot.get_conversation_messages(conv_id) do
                {:ok, msgs} ->
                  msg = msgs
                    |> Enum.reverse()
                    |> Enum.find(fn m ->
            ...
            # Fetch existing
            {cur_amt, cur_lbl} = case LipagasBridge.Chatwoot.get_last_cart_total(conv_id) do
            ...
            LipagasBridge.Chatwoot.post_note(conv_id, "CART_TOTAL:#{new_amt}|#{new_lbl}")
            ...
            LipagasBridge.Chatwoot.update_custom_attributes(conv_id, new_attrs)
            case LipagasBridge.Chatwoot.get_sender(conv_id) do
              {:ok, sender} -> 
                LipagasBridge.Chatwoot.update_contact(sender.id, new_attrs)
              _ -> :ok
            end
          _ -> :ok
        end
      ...
      String.contains?(combined_text, "[PROCESS_ORDER") ->
        ...
        {amount, label} = if amount_str == "" or parse_float(amount_str) <= 0 do
          case LipagasBridge.Chatwoot.get_last_cart_total(conv_id) do
            {:ok, val, lbl} -> {val, lbl}
            _ -> {0, "LipaGas Payment"}
          end
        else
          label = case LipagasBridge.Chatwoot.get_last_cart_total(conv_id) do
        ...
        meter = if meter == "" do
          case LipagasBridge.Chatwoot.get_conversation_messages(conv_id) do
            {:ok, msgs} ->
              msg = msgs
                |> Enum.reverse()
                |> Enum.find(fn m -> 
  ...
  def build_catalog_payload(combined_text, phone, msg_lower) do
    case Regex.run(~r/SHOW_CATALOG:\s*(\w+)/, combined_text) do
      [_, route_raw] ->
        ...
        sets  = Config.lipagas_catalog_sets()
  ...
  defp trigger_stk_push(phone, amount, label, meter, conv_id, retries \\ 3) do
    alias LipagasBridge.{Mpesa, Chatwoot}
    ...
    case Mpesa.fire_stk_push(formatted, amount, receipt, popup_text) do
      ...
      {:error, reason} ->
        ...
        if retries > 1 do
          ...
          LipagasBridge.Meta.send_text(phone, "⚠️ Safaricom's M-Pesa network is currently experiencing delays and did not respond. Please click the 'Retry' button in the previous message to try again.")
        end
    end
  end
