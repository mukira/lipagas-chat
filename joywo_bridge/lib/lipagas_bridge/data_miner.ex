defmodule LipagasBridge.DataMiner do
  use GenServer
  require Logger
  ...
  defp fetch_rss_data do
    ...
    items_by_source =
      ...
      |> Enum.map(fn {label, url} ->
        case LipagasBridge.HTTP.get(url) do
...
""" |> String.trim() |> String.slice(0, @max_context_chars)
  ...
  defp analyze_media_sentiment do
    ...
    case LipagasBridge.HTTP.post_json(url, payload, headers) do
  ...
  defp check_sentiment_with_gemini(summary) do
    ...
    case LipagasBridge.HTTP.post_json(url, body, []) do
  ...
  defp alert_pr_team(summary) do
    ...
    *Latest Media Snippets:*
    ...
    LipagasBridge.HTTP.post_json(url, payload, [{"Authorization", "Bearer #{token}"}])
  end
end
