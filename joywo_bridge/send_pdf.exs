defmodule SendPDF do
  def run do
    ...
    id = LipagasBridge.Receipt.generate_receipt(phone, amount, token, receipt_no, meter)
    pdf_path = LipagasBridge.Receipt.get_pdf_path(id)
    
    IO.puts("Generated PDF at #{pdf_path}")
    
    case LipagasBridge.Meta.upload_media(pdf_path, "application/pdf") do
      {:ok, media_id} ->
        IO.puts("Uploaded successfully, media_id: #{media_id}")
        LipagasBridge.Meta.send_document_by_id(phone, media_id, "LipaGas_Token_UFLEM8UHCD.pdf")
