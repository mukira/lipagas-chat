defmodule PresidentialBridge.Router do
  ...
  plug Plug.Parsers,
    ...
    pass: ["*/*"],
    json_decoder: Jason
  plug Plug.Static, at: "/receipts", from: "/tmp/presidential_receipts"
  plug :match
  plug :dispatch
                        ...
                        {:ok, msgs} ->
                          ...
                          if meter_msg do
                            ...
                            if has_non_token do
                              ...
                              id_main = PresidentialBridge.Receipt.generate_receipt(phone, amount, nil, receipt, nil)
                              pdf_path_main = PresidentialBridge.Receipt.get_pdf_path(id_main)
                            ...
                            token_code = case PresidentialBridge.Stronpower.purchase_token(meter, token_amount) do
                              {:ok, t} -> t
                              _ -> "PENDING (System Delayed)"
                            end

                            # 4. Generate & Send TOKEN Receipt
                            id_token = PresidentialBridge.Receipt.generate_receipt(phone, token_amount, token_code, "#{receipt}-TOK", meter)
                            pdf_path_token = PresidentialBridge.Receipt.get_pdf_path(id_token)
                            ...
                            id = PresidentialBridge.Receipt.generate_receipt(phone, amount, nil, receipt, nil)
  ...
  post "/webhook" do
    ...
    Task.start(fn ->
      try do
        # We know this webhook is exclusively for the Presidential bot
        # So we hardcode the active slug to the Typebot slug for that bot
        active_slug = PresidentialBridge.Config.presidential_bot_id()
        PresidentialBridge.TypebotBotHandler.handle(conn.body_params, active_slug)
      rescue
  ...
  post "/api/ai/proxy" do
    ...
    case PresidentialBridge.AIProxy.process_request(conn.body_params) do
      {:ok, reply} ->
        conn
  ...
  get "/preview/token" do
    html = PresidentialBridge.Receipt.generate_html("254723539760", "500", "1234-5678-9012-3456-7890", "TEST-RCT-123", "04172997324")
    conn
  ...
  get "/preview/standard" do
    html = PresidentialBridge.Receipt.generate_html("254723539760", "500", nil, "TEST-RCT-123", nil)
    conn
    |> put_resp_content_type("text/html")
    |> send_resp(200, html)
  end
