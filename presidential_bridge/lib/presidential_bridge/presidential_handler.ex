defmodule PresidentialBridge.PresidentialHandler do
  ...
  @reset_keywords ~w(reset hi hello start menu ruto)
  ...
  defp process_typebot(conv_id, incoming_msg, msg_lower, is_reset, session_id, event) do
    with {:ok, sender} <- Chatwoot.get_sender(conv_id),
      ...
      File.write("/tmp/presidential.log", "\n[DEBUG] attrs: #{inspect(attrs)}\n", [:append])
      variables_map = %{
      ...
      if effective_session_id && active_choices != [] && not is_valid_choice &&
        ...
        case PresidentialBridge.Typebot.start_chat(slug, %{"SystemEvent" => "INVALID_OPTION", "Phone" => format_phone(phone)}) do
        ...
        File.write("/tmp/presidential.log", "is_old_button: #{is_old_button}, effective_session: #{inspect(effective_session)}\n", [:append])
        ...
        {saved_location, short_loc, variables_map} =
          case Enum.find_value(tb_messages, nil, fn msg ->
            ...
            new_loc when is_binary(new_loc) and new_loc != "" ->
              ...
              Task.start(fn ->
                PresidentialBridge.Chatwoot.update_contact(sender.id, %{saved_location: new_loc})
                if is_nil(attrs["typebot_location"]) do
                  PresidentialBridge.Chatwoot.update_custom_attributes(conv_id, %{typebot_location: typebot_location})
                end
              end)
            ...
            {:ok, convs} ->
              ...
              if active_conv do
                ...
                if existing_session do
                  PresidentialBridge.InactivityTimer.cancel(conv_id)
