defmodule PresidentialBridge.Receipt do
  @receipts_dir "/tmp/presidential_receipts"

  def generate_html(phone, amount, token, receipt_no, meter \\ nil) do
    date = Calendar.strftime(Date.utc_today(), "%d %b %Y")
    {name, location} = case PresidentialBridge.Chatwoot.search_contact(phone) do
    ...
    items = case PresidentialBridge.Chatwoot.search_contact(phone) do
      {:ok, contact} when not is_nil(contact) ->
        case PresidentialBridge.Chatwoot.get_contact_conversations(contact["id"]) do
          {:ok, [conv | _]} ->
            case PresidentialBridge.Chatwoot.get_last_cart_total(conv["id"]) do
              {:ok, _, label} ->
                label
                |> String.split(", ")
...
<body>
        ...
        <tr>
          <td style="vertical-align: top; padding: 0;">
            ...
            <div class="std-logo-container">
              <img src="https://cdn.lipagas.co/bot/assets/presidential.svg" class="std-logo-img" alt="LipaGas" />
            </div>
  ...
  defp generate_qr_svg(data) do
    ...
    # These are the precise paths of the presidential-single.svg you provided
    logo_paths =
