defmodule PresidentialBridge.Session do
  @moduledoc """
  ...
  def get_session(conv_id) do
    case Redix.command(:redix, ["GET", "pres_tb_session:#{conv_id}"]) do
      {:ok, nil}     -> nil
      {:ok, val}     -> val
      {:error, _}    -> nil
    end
  ...
  def set_session(conv_id, session_id) do
    Redix.command(:redix, ["SET", "pres_tb_session:#{conv_id}", session_id, "EX", @session_ttl])
  end

  def delete_session(conv_id) do
    Redix.command(:redix, ["DEL", "pres_tb_session:#{conv_id}"])
  end

  # ─── Greeting Persistence (across internal resets) ────────────────────

  def get_greeting(conv_id) do
    case Redix.command(:redix, ["GET", "pres_greeting:#{conv_id}"]) do
      {:ok, nil}  -> nil
      {:ok, val}  -> val
      {:error, _} -> nil
    end
  ...
  def set_greeting(conv_id, greeting) do
    # TTL of 24 hours (86400 seconds) so it doesn't stay forever
    Redix.command(:redix, ["SET", "pres_greeting:#{conv_id}", greeting, "EX", 86400])
  end

  def delete_greeting(conv_id) do
  ...
  def get_button_mapping(slug) do
    case Redix.command(:redix, ["GET", "pres_button_map:#{slug}"]) do
      {:ok, nil} -> nil
      {:ok, val} ->
  ...
  def set_button_mapping(slug, mapping) when is_map(mapping) do
    # TTL of 1 hour (3600 seconds) to ensure buttons are refreshed regularly
    Redix.command(:redix, ["SET", "pres_button_map:#{slug}", Jason.encode!(mapping), "EX", 3600])
  end

  # ─── Active Choices (for invalid option detection) ───────────────────

  def get_active_choices(conv_id) do
    case Redix.command(:redix, ["GET", "pres_choices:#{conv_id}"]) do
      {:ok, nil} -> []
      {:ok, val} ->
        case Jason.decode(val) do
          {:ok, list} -> list
  ...
  def set_active_choices(conv_id, choices) when is_list(choices) do
    Redix.command(:redix, ["SET", "pres_choices:#{conv_id}", Jason.encode!(choices), "EX", @session_ttl])
  end

  # ─── Category tracking ───────────────────────────────────────────────

  def get_category(conv_id) do
    case Redix.command(:redix, ["GET", "pres_category:#{conv_id}"]) do
      {:ok, nil}  -> ""
      {:ok, val}  -> val
      {:error, _} -> ""
    end
  ...
  def set_category(conv_id, category) do
    Redix.command(:redix, ["SET", "pres_category:#{conv_id}", category, "EX", @session_ttl])
  end
  ...
  def get_waiting_location(conv_id) do
    case Redix.command(:redix, ["GET", "pres_loc_wait:#{conv_id}"]) do
      {:ok, val} -> val == "true"
      _ -> false
    end
  end
  ...
  def set_waiting_location(conv_id, val) when is_boolean(val) do
    if val do
      Redix.command(:redix, ["SET", "pres_loc_wait:#{conv_id}", "true", "EX", 300]) # 5 mins
    else
      Redix.command(:redix, ["DEL", "pres_loc_wait:#{conv_id}"])
    end
  end
end
