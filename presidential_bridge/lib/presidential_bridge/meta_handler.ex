defmodule PresidentialBridge.MetaHandler do
  @moduledoc """
  ...
  @presidential_phone_id Config.phone_id()
        ...
        cond do
          ...
          true ->
            if message["type"] == "interactive" and get_in(message, ["interactive", "type"]) == "nfm_reply" do
              PresidentialBridge.PresidentialHandler.handle_nfm_reply(phone, message)
            end
  ...
  defp fetch_catalog_data do
    ...
    case PresidentialBridge.HTTP.get(url, [{"Authorization", "Bearer #{Config.meta_token()}"}]) do
